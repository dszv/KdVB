...
real, parameter :: vel = 4.7
real, parameter :: x0 = 30.0
real R0(nn)
...
real state(nn), omega_span(50), phase_init_span(50)
...
call evol_point(vel, R0, amp_vec)
...
subroutine evol_point(vel, R0, amp_vec)
! real, intent(in) :: omega
! real, intent(in) :: phase_init
real, intent(in) :: vel
real, dimension(nn), intent(in) :: R0
...
call init_KdV_soliton(state_evol, vel, R0, r)
...
subroutine radial_grid(r)
real red(nn)
! real, dimension(nn), intent(out) :: r, gamma
real, dimension(nn), intent(out) :: r
integer j, i
...
subroutine init_KdV_soliton(state, vel, R0, r)
...
! w = 0.0
sech_aux = 1.0/cosh(sqrt(vel)*(r - R0)/2) !possibly?
KdV_soliton = (vel/2.0)*(sech_aux**2)
do i = 1, nn
    ! state(i) = SG_breather(i)
    state(i) = KdV_soliton(i)
    ! state(nn+i) = SG_breather_t(i)
    ! state(2*nn+i) = SG_breather_r(i)/r(i)
    ! state(3*nn+i) = 0.0
end do
...
! if (mark == 1) open(unit = 22, file = 'breather.bin', access='stream', status='unknown')
if (mark == 1) open(unit = 22, file = 'soliton.bin', access='stream', status='unknown')
! if (mark> 1) open(unit = 22, file = 'breather.bin', access='stream', status='old', position = 'append')
