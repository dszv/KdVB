...
if (mark == 1) open(unit = 22, file = 'phi1_VI.bin', access='stream', status='unknown')
if (mark> 1) open(unit = 22, file = 'phi1_VI.bin', access='stream', status='old', position = 'append')
