...
print *, "linearized solution, part II"
do m = 4, 6
! vel = m*0.1
forall (i=1:NN) v(i) = 0.0
call dump_sol(v, m-3, 0.0)
print *, "vel:", m*0.1
do j = 1, TT
...
if (mark == 1) open(unit = 22, file = 'phi1_II.bin', access='stream', status='unknown')
if (mark> 1) open(unit = 22, file = 'phi1_II.bin', access='stream', status='old', position = 'append')
